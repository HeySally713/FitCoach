...
const pickForPatterns = (
  ...
  const ranked = pool
    ...
    .map(ex => ({ ex, score: scoreExercise(ex, goal, preferredParts) }))
  ...
  // Second pass: fill remaining slots with next best
  for (const { ex } of ranked) {
    if (picked.length >= count) break;
...
const buildDay = (
  ...
  const picks = pickForPatterns(pool, patterns, count, goal, preferredParts, excludeIds);
  ...
  if (splitType === 'flexibility_focus') {
    ...
    for (let i = 0; i < frequency; i++) {
      ...
      if (cardioPerSession > 0) {
        const cardioPicks = pickForPatterns(pool, PATTERN_GROUPS.cardio, 1, goal, preferredParts, used);
